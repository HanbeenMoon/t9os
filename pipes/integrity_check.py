...
PIPELINE_REGISTRY = {p["file"]: {"desc": p["desc"], "path": p.get("path")} for p in _REG}
...
def check_entity_consistency():
    ...
    try:
        ...
        for row in db_entities:
            ...
            # filepath can be relative to T9OS or to HANBEEN root
            full_path = BASE / fp
            if not full_path.exists():
                full_path = HANBEEN_ROOT / fp
            if not full_path.exists():
                orphans.append({
...
def check_pipeline_status():
    ...
    for filename, info in PIPELINE_REGISTRY.items():
        description = info["desc"]
        filepath = info.get("path") or (PIPES_DIR / filename)
        entry = {
            "name": filename,
