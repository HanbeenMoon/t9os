"""T9 OS Session Lock — 프로젝트/파일 단위 충돌 방지 (구체화 v2)

v2 변경 (2026-03-26 구체화):
- 세션 관리/heartbeat → lib/ipc.py로 위임 (단일 소스)
- 이 파일은 **프로젝트 claim 전문가**로 역할 축소
- 프로젝트 패턴 → config/projects.json 단일 소스
...
def _get_project_patterns(project: str) -> list[str]:
    """프로젝트에 해당하는 파일 패턴을 반환. config/projects.json 단일 소스 (구체화 v2)."""
    # projects.json 우선
        ...
        try:
            ...
            for key, info in projects.items():
                if key.upper() == project.upper():
                    return info.get("file_patterns", [])
        except (json.JSONDecodeError, OSError):
            pass
    # 폴백: 하드코딩 패턴 (구 방식, 침전 예정)
...
def heartbeat(description: str = "") -> None:
    """세션 heartbeat 갱신 — ipc.py에 위임 (구체화 v2)."""
    try:
        import sys
        sys.path.insert(0, str(T9))
        from lib.ipc import heartbeat_update
        sid = _get_session_id()
        heartbeat_update(sid)
    except Exception:
        pass  # ipc.py 로드 실패 시 무시 (graceful)
...
def sync_working_md() -> None:
    """WORKING.md 동기화 — ipc.py에 위임 (구체화 v2). [SESSIONS] → [AUTO]로 통합."""
    try:
        import sys
        sys.path.insert(0, str(T9))
        from lib.ipc import sync_working_md as ipc_sync
        ipc_sync()
    except Exception:
        pass  # ipc.py 로드 실패 시 무시
