...
def run_auto(dry_run=False):
    ...
    if not dry_run:
        ...
        # ── 5.5. 마감 정제 (Gemini로 제목+날짜 추출) ──
        needs_deadline = cursor.execute(
            "SELECT id, filename, body_preview FROM entities "
            "WHERE phase IN ('preindividual', 'tension_detected') "
            "AND deadline_date IS NULL "
            "AND (body_preview LIKE '%마감%' OR body_preview LIKE '%제출%' "
            "     OR body_preview LIKE '%발표%' OR body_preview LIKE '%시험%' "
            "     OR filename LIKE '%마감%' OR filename LIKE '%제출%' "
            "     OR filename LIKE '%발표%') "
            "LIMIT 10"
        ).fetchall()
        ...
        if needs_deadline:
            items_text = "\n".join(
                f'{r["id"]}: {(r["body_preview"] or r["filename"])[:100]}'
                for r in needs_deadline
            )
            dl_prompt = (
                f"각 항목에서 마감/제출/발표 날짜와 깔끔한 제목을 추출해. "
                f"오늘은 {now:%Y-%m-%d}이야. D-N이면 오늘 기준으로 날짜 계산해.\n"
                f"JSON 배열로만 답해. 날짜 없으면 date를 null로.\n"
                f'형식: [{{"id": N, "title": "깔끔한 제목", "date": "YYYY-MM-DD"}}, ...]\n\n'
                f"{items_text}"
            )
            dl_raw = gemini_call(dl_prompt, max_tokens=80 * len(needs_deadline))
            if dl_raw:
                try:
                    dl_match = re.search(r'\[.*\]', dl_raw, re.DOTALL)
                    if dl_match:
                        dl_parsed = json.loads(dl_match.group())
                        for item in dl_parsed:
                            if isinstance(item, dict) and "id" in item:
                                ...
                                if date and re.match(r'\d{4}-\d{2}-\d{2}', date):
                                    cursor.execute(
                                        "UPDATE entities SET deadline_date=?, updated_at=? WHERE id=?",
                                        (date, now.isoformat(), eid)
                                    )
                                if title:
                                    # 파일명은 안 바꾸고 metadata에 정제된 제목 저장
                                    row = cursor.execute("SELECT metadata FROM entities WHERE id=?", (eid,)).fetchone()
                                    try:
                                        meta = json.loads(row[0]) if row and row[0] else {}
                                    except (json.JSONDecodeError, TypeError):
                    ...
                    pass
            print(f"  마감 정제 대상: {len(needs_deadline)}건")
