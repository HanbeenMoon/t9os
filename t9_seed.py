...
from lib.commands_ext import (
    ...
    cmd_orphans as _cmd_orphans, cmd_rebuild_vectors as _cmd_rebuild_vectors,
)
...
USAGE = """  사용법: python3 t9_seed.py <command> [args]
  ...
  rebuild-vectors       벡터 임베딩 전체 재구축 (gemini-embedding-001)
...
_EXTRA_COLUMNS = {
    ...
    "embedding":  "BLOB",      # numpy float32 (1536dim). 벡터 공간 좌표. gravity.py 참조.
}
...
def _migrate_db(conn):
    ...
    # sqlite-vec 벡터 테이블 (Phase 1 공간 인프라)
    try:
        from lib.vec import load_vec
        load_vec(conn)
        conn.execute("""CREATE VIRTUAL TABLE IF NOT EXISTS entity_vectors
                        USING vec0(entity_id INTEGER PRIMARY KEY, embedding FLOAT[3072])""")
    except Exception:
        pass  # sqlite-vec 미설치 시 벡터 기능 비활성화
    conn.commit()
...
def get_db():
    ...
    # 마이그레이션: 정규 컬럼 + relates 테이블 + 벡터
    _migrate_db(conn)
    # sqlite-vec 로드 (매 연결마다 필요)
    try:
        from lib.vec import load_vec
        load_vec(conn)
    except Exception:
...
def _upsert(conn, rel, fname, phase, meta_json, preview, h, full_body="",
    ...
    # TD-NEW1: filepath 타입 검증 — DB 오염 방지
    if not isinstance(rel, str) or not rel.strip():
        print(f"  [WARN] 잘못된 filepath 무시: {rel!r}")
        return False
...
def cmd_capture(text):
    ...
    # ── 벡터 임베딩 (Phase 1 공간 인프라) ──
    try:
        from lib.vec import get_embedding, upsert_vector, vec_available, build_embed_text
        conn = get_db()
        if vec_available(conn):
            rel = str(filepath.relative_to(T9))
            row = conn.execute("SELECT id, concepts, phase FROM entities WHERE filepath=?", (rel,)).fetchone()
            if row:
                embed_text = build_embed_text(text, row["concepts"] or "", row["phase"] or "")
                emb = get_embedding(embed_text)
                if emb:
                    upsert_vector(conn, row["id"], emb)
                    conn.commit()
        conn.close()
    except Exception as e:
        _log.debug("capture embedding skipped: %s", e)
    ...
    # ── 중력: 개체화는 capture 순간 시작된다 ──
    try:
        from lib.gravity import gravitational_capture
        conn = get_db()
        rel = str(filepath.relative_to(T9))
        row = conn.execute("SELECT id FROM entities WHERE filepath=?", (rel,)).fetchone()
        if row:
            g_result = gravitational_capture(conn, row["id"], text, rel)
            if g_result.get("embedded"):
                _log.debug("gravity: neighbors=%d relates=%d propagated=%d",
                           g_result["neighbors"], g_result["relates"], g_result["propagated"])
        conn.close()
    except Exception as e:
        _log.debug("gravity skipped: %s", e)
        ...
        if raw:
            ...
            if _m:
                ...
                if row:
                    ...
                    # deadline_date는 마감 키워드가 있을 때만 설정 (오탐 방지)
                    # 세션 데이터/캘린더 동기화는 마감이 아니므로 제외
                    _fn = row["filename"] if "filename" in row.keys() else ""
                    _bp = row["body_preview"] if "body_preview" in row.keys() else ""
                    _is_session = (_bp or "").startswith("# CC Session") or (_fn or "").endswith("_brief.md")
                    _is_calendar = "GoogleCalendar" in (_fn or "")
                    if (parsed.get("date") and parsed["date"] != "null"
                            and re.match(r'\d{4}-\d{2}-\d{2}', str(parsed["date"]))
                            and not _is_session and not _is_calendar
                            and re.search(r'마감|제출|시험|고사|deadline|due', (_fn or "") + (_bp or ""), re.IGNORECASE)):
    ...
    # ── 디스패처: 멀티세션 자동 배분 (활성 세션 2개 이상일 때만) ──
    try:
        from lib.ipc import heartbeat_who
        alive = heartbeat_who()
        if len(alive) >= 2:
            from pipes.dispatcher import classify_project, dispatch
            project = classify_project(text)
            # 현재 세션이 이미 해당 프로젝트를 담당 중이면 dispatch 안 함
            current_sid = ""
            if SESSION_FILE.exists():
                current_sid = SESSION_FILE.read_text().strip()
            conn = get_db()
            row = conn.execute("SELECT claimed_project FROM sessions WHERE id=?", (current_sid,)).fetchone()
            conn.close()
            current_project = row[0] if row and row[0] else ""
            if current_project.upper() != project.upper():
                dispatch(text, urgency or "normal")
    except Exception as e:
        _log.debug("dispatcher skipped: %s", e)
...
def cmd_reindex(incremental=False):
    ...
    # 임베딩 부트스트랩: embedding이 없는 엔티티에 벡터 생성
    try:
        from lib.gravity import embed_batch
        no_embed = conn.execute(
            "SELECT id, body_preview FROM entities WHERE embedding IS NULL AND body_preview IS NOT NULL AND body_preview != ''"
        ).fetchall()
        if no_embed:
            batch_size = 100
            embedded = 0
            for i in range(0, len(no_embed), batch_size):
                batch = no_embed[i:i+batch_size]
                texts = [r["body_preview"][:500] for r in batch]
                vecs = embed_batch(texts)
                for r, v in zip(batch, vecs):
                    if v is not None:
                        conn.execute("UPDATE entities SET embedding=? WHERE id=?", (v.tobytes(), r["id"]))
                        embedded += 1
                conn.commit()
            if embedded > 0:
                print(f"  임베딩 생성: {embedded}건")
    except Exception as e:
        _log.warning("embedding bootstrap failed: %s", e)
    conn.close()
    self_check("reindex")
...
def cmd_search(query):
    conn = get_db()
    # 1단계: LIKE 검색 (한글 완벽 지원, 최신+관련도 정렬)
    ...
    like_ids = {r["id"] for r in results}

    # 2단계: 벡터 검색 (가능하면)
    vec_results = []
    try:
        from lib.vec import get_query_embedding, search_vectors, vec_available
        if vec_available(conn):
            qemb = get_query_embedding(query)
            if qemb:
                vec_hits = search_vectors(conn, qemb, limit=20)
                new_ids = [eid for eid, dist in vec_hits if eid not in like_ids and dist < 1.2]
                if new_ids:
                    placeholders = ",".join("?" * len(new_ids))
                    vec_results = conn.execute(
                        f"SELECT id,filename,phase,metadata,urgency,concepts FROM entities "
                        f"WHERE id IN ({placeholders})", new_ids).fetchall()
    except Exception:
        pass  # 벡터 검색 실패 시 LIKE 결과만

    # 3단계: 출력
    all_results = list(results) + list(vec_results)
    vec_only_ids = {r["id"] for r in vec_results}
    if not all_results: print("  결과 없음")
    else:
        for r in all_results:
            ...
            vec_mark = " ~" if r["id"] in vec_only_ids else ""
            print(f"  [{r['id']:3d}] {r['phase'][:12]:12s} | {r['filename'][:50]:50s} | {tags}{urg_mark}{vec_mark}")
    conn.close()
...
def cmd_transition(eid, to_phase, reason=""):
    ...
    if is_protected:
        ...
        # TD-011: 이중파일 감지 — copy+delete 후 원본 잔존 확인
        if old_path != new_path and old_path.exists() and new_path.exists():
            print(f"  [WARN] 이중파일 감지! 원본 잔존: {old_path}")
            print(f"         새 파일: {new_path}")
            print(f"         원본을 수동으로 삭제하세요.")
        new_rel = str(new_path.relative_to(T9))
...
def cmd_rebuild_vectors():
    _cmd_rebuild_vectors(get_db)
...
def main():
    ...
    elif c in ("rebuild-vectors","rebuild_vectors"): cmd_rebuild_vectors()
