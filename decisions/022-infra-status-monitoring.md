...
- 날짜: 2026-03-05
- 상태: 채택됨
- 결정: Dashboard에 `/api/infra` 라우트를 추가하여 GitHub, Notion, 서울PC, 세종PC의 연결 상태를 실시간으로 모니터링한다. 각 서비스에 헬스체크 요청을 보내고 응답 여부로 online/offline 판단.
- 이유:
  - T9 OS는 여러 외부 서비스(GitHub, Notion, Google Calendar)와 2대의 PC(서울, 세종)에 의존한다.
...
- 결과:
  - Dashboard 상단에 인프라 상태 표시 (GitHub, Notion, 서울PC, 세종PC).
