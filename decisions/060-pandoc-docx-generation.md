...
- 이유: python-docx로 표/서식을 코드 생성하면 서식이 깨짐(SSK v6 실패). pandoc은 Markdown 원본을 유지하면서 안정적으로 DOCX 변환 가능. libreoffice --headless로 PDF 동시 생성.
- 대안: python-docx (서식 깨짐), LaTeX (한글 설정 복잡), docxtpl (표 복잡 구조 한계)
- 결과: SSK v7~v25 전량 pandoc 생성, PM3 파이프라인 표준
- 출처: 20260316_CC_001_015111_SSK논문재생성_T9OS정비.txt, memory/feedback_always_pdf.md
