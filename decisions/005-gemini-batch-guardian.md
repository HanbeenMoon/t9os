...
- 이유:
  - cc(Claude Code)의 토큰은 비싸고 제한적이다 ("37만원짜리 맥스 20을 써도 대학생이 토큰이 부족한게 말이되냐?"). 감시단 21명을 cc 토큰으로 실행하면 실제 작업에 쓸 토큰이 부족해진다.
  ...
  - 결과는 `_ai/logs/gm/{timestamp}_guardian_brief.md` (CEO 브리프)로 자동 생성되어 한빈은 P0만 읽으면 된다.
- 대안:
