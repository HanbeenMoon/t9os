...
- 이유: 단일 LLM 리뷰는 맹점이 있음. 3개 모델의 교차 검증으로 하드코딩 토큰, 파싱 버그 등을 cc 단독으로는 놓치는 문제를 발견. 특히 크리덴셜 하드코딩(P0)을 octopus가 포착.
- 대안: cc 단독 리뷰 (맹점), 수동 리뷰 (시간 소모), GitHub Actions CI (설정 복잡)
- 결과: L1 + memory에 "코드 리뷰 자율 실행" 규칙 추가, orchestrate.sh 파싱 버그 발견+패치
- 출처: 20260316_CC_003_041500_T9OS_v02_시몽동개정_model-project_project-alpha.txt, 20260317_e7c9a78f.md
