...
> 한빈이 기술 배경 없이도 한눈에 볼 수 있는 전체 그림.

## 전체 흐름

```mermaid
graph TB
    subgraph INPUT["입력 (한빈이 뭔가를 던짐)"]
        카톡["카톡 나에게보내기"]
        음성["음성 메모"]
        영감["아이디어/영감"]
        지시["직접 지시"]
    ...
    subgraph FIELD["Layer A: 전개체 저장소 (field/)"]
        inbox["inbox/<br/>아직 뭔지 모르는 것들<br/>274건+"]
...
stateDiagram-v2
    ...
    candidate_generated --> individuating: 한빈 승인
...
graph TB
    subgraph L3["L3 개정 규칙 (언제 규칙을 바꾸나)"]
        trigger["트리거: 현실 불일치 3회+<br/>한빈 지시 / 새 도구 발견"]
...
graph LR
    ...
    style T3 fill:#636e72,color:#fff
```
