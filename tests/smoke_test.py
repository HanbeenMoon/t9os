#!/usr/bin/env python3
"""
T9 OS Smoke Tests — pipeline min check
execution: python3 T9OS/tests/smoke_test.py
"""
...
try:
    ...
    check(" ", count > 0, f"count={count}")

    # FTS5 test
    try:
        conn.execute("SELECT * FROM entities_fts LIMIT 1")
        check("FTS5 table ", True)
    except Exception as e:
        check("FTS5 table ", False, str(e))
    ...
    check("required column ", not missing, f"missing={missing}" if missing else "")
    ...
    check("filepath  consistency", bad == 0, f"corrupted={bad}")
...
check("status command", rc == 0 and "T9 OS Seed" in out, err[:100] if rc else "")

rc, out, err = run_cmd(["python3", "t9_seed.py", "search", "test"])
check("search command", rc == 0, err[:100] if rc else "")

rc, out, err = run_cmd(["python3", "t9_seed.py", "orphans"])
check("orphans command", rc == 0, err[:100] if rc else "")
...
if rc == 0:
    ...
    skip("cron", "crontab access ")

# 5. systemd service
print("\n  --- systemd ---")
rc, out, err = run_cmd(["systemctl", "--user", "is-active", "t9_bot"])
check("t9_bot service active", rc == 0 and "active" in out, out.strip() if rc else err[:50])

# 6. Config imports
print("\n  --- module ---")
rc, out, err = run_cmd(["python3", "-c", "from lib.config import *; print('OK')"])
check("lib.config ", rc == 0 and "OK" in out, err[:100] if rc else "")

rc, out, err = run_cmd(["python3", "-c", "from lib.logger import *; print('OK')"])
check("lib.logger ", rc == 0 and "OK" in out, err[:100] if rc else "")

rc, out, err = run_cmd(["python3", "-c", "from lib.parsers import *; print('OK')"])
check("lib.parsers ", rc == 0 and "OK" in out, err[:100] if rc else "")
