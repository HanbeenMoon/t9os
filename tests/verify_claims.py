...
check("t9_seed.py 줄수", 893, actual_lines, tolerance=30)

# 2. pipes/ 파이프라인 수
actual_pipes = len(list((T9 / "pipes").glob("*.py")))
check("pipes/*.py 수", 27, actual_pipes, tolerance=3)

# 3. lib/ 모듈 수
actual_lib = len(list((T9 / "lib").glob("*.py")))
check("lib/*.py 수", 13, actual_lib, tolerance=2)
...
check("inbox 파일 수", 789, actual_inbox, tolerance=150)
...
try:
    ...
    if match:
        ...
        check("smoke test PASS 수", 47, actual_checks, tolerance=3)
except Exception:
    pass
...
try:
    ...
    if r.returncode == 0:
        ...
        check("cron 등록 수", 23, len(cron_lines), tolerance=3)
except Exception:
    pass
