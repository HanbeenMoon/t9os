...
def _parse_deadlines(find_deadline_file_fn=None, conn=None):
    """마감일 파싱. DB에서 직접 읽기 (Notion 의존 제거).
    conn이 주어지면 DB에서, 아니면 레거시 파일 fallback."""
    ...
    if conn is not None:
        try:
            ...
            for r in rows:
                try:
                    days = (datetime.strptime(r["deadline_date"], "%Y-%m-%d").date() - today).days
                    if -1 <= days <= 14:
                        import re, json as _j
                        name = None
                        try:
                            meta = _j.loads(r["metadata"]) if r["metadata"] else {}
                        ...
                        urgent = r["urgency"] in ("critical", "high") if r["urgency"] else False
                        deadlines.append({"title": name, "date": r["deadline_date"], "days": days, "urgent": urgent})
    ...
    # 레거시 fallback (Notion dump 파일)
    if find_deadline_file_fn:
        ...
        for block in blocks:
            ...
            for line in block.split("\n"):
                if ":" in line:
                    ...
                    d[k.strip()] = v.strip()
            if "날짜" in d and "제목" in d and d.get("완료", "N") != "Y":
                try:
...
def cmd_daily(get_db, find_deadline_file_fn):
    ...
    dl = _parse_deadlines(find_deadline_file_fn, conn=conn)
    if dl:
        print("  [!] 마감일:")
        for d in dl:
    ...
    if active:
        print(f"  진행 중: {len(active)}건 (t9_search로 조회)")
        print()
    ...
    if stale:
        print(f"\n  [?] 30일+ 중단: {len(stale)}건 (t9_search로 조회)")
            ...
            if concepts:
                ...
                for c in concepts:
                    related_count += conn.execute(
                        "SELECT COUNT(*) as cnt FROM entities WHERE phase IN ('individuating','tension_detected') "
                        "AND (concepts LIKE ? OR metadata LIKE ?) AND id!=?",
                        (f'%"{c}"%', f'%"{c}"%', r["id"])
                    ).fetchone()["cnt"]
                print(f"    [{r['id']}] {r['filename'][:40]} → {', '.join(concepts)} (전도 가능: {related_count}건)")
